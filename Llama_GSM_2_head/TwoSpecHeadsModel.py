...
class ThreeHeadModel(torch.nn.Module):
    def __init__(self, base_model, speculator_head_one, speculator_head_two):
    ...
    def forward(self, x, attention_mask=None):
        ...
        hidden_states = self.base_model.model(x, attention_mask=attention_mask).last_hidden_state
        out1 = self.main_head(hidden_states)
        out2 = self.speculator_head_one(hidden_states)
        out3 = self.speculator_head_two(hidden_states)
